...
def getBearerToken2(auth_code):
    ...
    payload = {
        'code': auth_code,
        'redirect_uri': settings.REDIRECT_URI2,
        'grant_type': 'authorization_code'
    }
    r = requests.post(token_endpoint, data=payload, headers=headers)
    if r.status_code != 200:
...
def getProfitAndLoss(access_token, realmId, startDate, endDate, accounting_method):
    route = '/v3/company/{0}/reports/ProfitAndLoss?start_date={1}&end_date={2}&accounting_method={3}&summarize_column_by=Month&minorversion=51'.format(realmId, startDate, endDate, accounting_method)
    print('[SPIDER] [Service] ', route)
    auth_header = 'Bearer ' + access_token
    headers = {'Authorization': auth_header, 'accept': 'application/json'}
    r = requests.get(settings.SANDBOX_QBO_BASEURL + route, headers=headers)
    status_code = r.status_code
...
def getBalanceSheet(access_token, realmId, startDate, endDate, accounting_method):
    route = '/v3/company/{0}/reports/BalanceSheet?start_date={1}&end_date={2}&accounting_method={3}&summarize_column_by=Month&minorversion=51'.format(realmId, startDate, endDate, accounting_method)
    print('[SPIDER] [Service] ', route)
    auth_header = 'Bearer ' + access_token
    headers = {'Authorization': auth_header, 'accept': 'application/json'}
    r = requests.get(settings.SANDBOX_QBO_BASEURL + route, headers=headers)
...
def getAccountListDetail(access_token, realmId):
    route = '/v3/company/{0}/query?query={1}'.format(realmId, 'select * from Account MaxResults 1000')
    print('[SPIDER] [Service] ', route)
    auth_header = 'Bearer ' + access_token
    headers = {'Authorization': auth_header, 'accept': 'application/json'}
    r = requests.get(settings.SANDBOX_QBO_BASEURL + route, headers=headers)
